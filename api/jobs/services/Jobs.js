...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Jobs
      .findOne(_.pick(params, _.keys(Jobs.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Jobs.create(data);
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Jobs.associations.map(a => a.alias));
    const data = _.omit(values, Jobs.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Jobs.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Jobs.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Jobs.attributes).reduce((acc, curr) => {
      switch (Jobs.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
